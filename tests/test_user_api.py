...
def test_home(base_url):
    res = requests.get(f"{base_url}/user/")  # 修复这里！
...
def test_register_user_success(base_url, headers):
    ...
    res = requests.post(f"{base_url}/user/register", json=data, headers=headers)  # 修复这里！
    assert res.status_code == 200
...
def test_get_user_success(base_url):
    res = requests.get(f"{base_url}/user/42")
    assert res.status_code == 200
...
def test_update_user(base_url, headers):
    update_data = {"age": 20}
    res = requests.put(f"{base_url}/user/42", json=update_data, headers=headers)
    assert res.status_code == 200
    assert res.json()["msg"] == "修改成功"

# 8 删除用户
def test_delete_user(base_url):
    res = requests.delete(f"{base_url}/user/42")
    assert res.status_code == 200
